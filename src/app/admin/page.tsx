...
export default function AdminPage() {
    ...
    useEffect(() => {
        ...
        if (savedTab && (savedTab === 'products' || savedTab === 'flowers' || savedTab === 'settings')) {
    ...
    const toggleShopOpen = async () => {
        ...
        try {
            // Find existing settings row first (same as handleSaveSettings)
            const { data: existing } = await supabase.from('settings').select('id').limit(1).single();
            if (!existing) throw new Error('Settings row not found');
            const { error } = await supabase
                .from('settings')
                .update({ shop_open: newValue })
                .eq('id', existing.id);
            if (error) throw error;
            showNotif(newValue ? 'Магазин открыт' : 'Магазин закрыт', 'success');
        } catch (err: any) {
            console.error('Toggle shop_open error:', err);
            setSettings({ ...settings, shop_open: !newValue }); // revert
            showNotif('Ошибка: не удалось сохранить статус магазина', 'error');
        }
    };
    ...
    const toggleDelivery = async () => {
        ...
        try {
            ...
            setSettings({ ...settings, delivery_enabled: !newValue }); // revert
            showNotif('Ошибка: не удалось сохранить статус доставки', 'error');
        }
    };
