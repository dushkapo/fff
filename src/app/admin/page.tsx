...
import { LogOut, Plus, Trash2, Edit, Package, Settings, Upload, X, Save, Store, StoreIcon, Loader2 } from 'lucide-react';
import { supabase, Product, Settings as SettingsType } from '@/lib/supabase';
...
export default function AdminPage() {
    ...
    const loadData = async () => {
        ...
        if (settingsRes.data) {
            ...
            setSettings({
                ...
                shop_name: 'Diana Flowers',
                hero_title: 'DIANA FLOWERS',
                hero_subtitle: 'Изысканные букеты для особых моментов',
                ...
                about_text: '',
                delivery_price: '500 ₽',
                delivery_info: 'Срочная за 2 часа',
                pickup_info: 'Бесплатно',
                payment_info: 'Наличные, карта',
            });
        }
        setIsLoading(false);
    };
    ...
    const handleSaveSettings = async () => {
        ...
        // If no ID, insert new settings, otherwise update
        if (!settings.id) {
    ...
    return (
        <div className="min-h-screen bg-gray-100">
            ...
            {/* Notification */}
            {notification && (
                <div className={`fixed top-4 right-4 z-50 px-6 py-3 rounded-lg text-white font-medium shadow-lg animate-pulse
            ...
            <main className="max-w-6xl mx-auto px-4 py-6">
                ...
                <div className="flex gap-2 mb-6">
                    <button
                        ...
                        className={`flex items-center gap-2 px-6 py-3 rounded-lg font-semibold transition-all ${activeTab === 'products' ? 'bg-zinc-900 text-white' : 'bg-white text-zinc-700 hover:bg-zinc-100'
                            }`}
                    >
                        <Package size={20} />
                        Букеты
                    ...
                    <button
                        ...
                        className={`flex items-center gap-2 px-6 py-3 rounded-lg font-semibold transition-all ${activeTab === 'settings' ? 'bg-zinc-900 text-white' : 'bg-white text-zinc-700 hover:bg-zinc-100'
                            }`}
                    >
                        <Settings size={20} />
                        Настройки
                ...
                {activeTab === 'settings' && settings && (
                    <div className="space-y-6">
                        ...
                        {/* Content Section */}
                        <div className="bg-white rounded-xl p-6 shadow-lg">
                            <h2 className="text-xl font-bold mb-4">Контент сайта</h2>
                            <div className="space-y-4">
                                <div>
                                    <label className="block text-sm font-medium mb-1">Текст «О нас»</label>
                                    <textarea
                                        value={settings.about_text || ''}
                                        onChange={(e) => setSettings({ ...settings, about_text: e.target.value })}
                                        placeholder="Diana Flowers — это премиальный цветочный бутик..."
                                        rows={4}
                                        className="w-full px-4 py-3 border border-zinc-200 rounded-lg focus:border-amber-500 focus:outline-none resize-none"
                                    />
                                </div>
                                <div className="grid md:grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        <label className="block text-sm font-medium mb-1">Информация о доставке</label>
                                        <input
                                            type="text"
                                    ...
                                    <div>
                                        <label className="block text-sm font-medium mb-1">Самовывоз</label>
                                        <input
                                            type="text"
