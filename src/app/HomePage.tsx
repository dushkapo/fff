                ...
                {mobileMenuOpen && (
                    <div className="md:hidden bg-white py-4 border-t border-zinc-100">
                        <div className="container mx-auto px-6 flex flex-col gap-4">
                            <a href="#catalog" className="text-zinc-600 hover:text-zinc-900 text-sm uppercase tracking-wider">{t('catalog')}</a>
                            {settings?.about_text && (
                                <a href="#about" className="text-zinc-600 hover:text-zinc-900 text-sm uppercase tracking-wider">{t('about')}</a>
                            )}
                            <a href="#delivery" className="text-zinc-600 hover:text-zinc-900 text-sm uppercase tracking-wider">{t('delivery')}</a>
                            <a href="#contacts" className="text-zinc-600 hover:text-zinc-900 text-sm uppercase tracking-wider">{t('contacts')}</a>
            ...
            <section id="delivery" className="py-16 bg-white">
                <div className="container mx-auto px-6">
                    <h2 className="text-xl uppercase tracking-[0.2em] text-zinc-800 text-center mb-10">
                        {settings?.delivery_enabled !== false ? t('deliveryAndPayment') : t('pickupAndPayment')}
                    </h2>
                    ...
                    <div className={`grid gap-8 max-w-4xl mx-auto ${settings?.delivery_enabled !== false ? 'md:grid-cols-3' : 'md:grid-cols-2'}`}>
                        ...
                        {settings?.delivery_enabled !== false && (
                            <div className="text-center p-8 bg-zinc-50 rounded-lg">
                                ...
                                <h3 className="font-semibold text-zinc-800 mb-2">{t('deliveryOption')}</h3>
                                {settings?.delivery_price_enabled && settings?.delivery_price && (
                                    <p className="text-zinc-600 text-sm">{t('deliveryInCity')} — {settings.delivery_price}</p>
                                )}
                        ...
                        <div className="text-center p-8 bg-zinc-50 rounded-lg">
                            <div className="text-4xl mb-4">🏪</div>
                            <h3 className="font-semibold text-zinc-800 mb-2">{t('pickup')}</h3>
                        ...
                        <div className="text-center p-8 bg-zinc-50 rounded-lg">
                            <div className="text-4xl mb-4">💳</div>
                            <h3 className="font-semibold text-zinc-800 mb-2">{t('payment')}</h3>
            ...
            <footer id="contacts" className="py-16 bg-zinc-900 text-white">
                <div className="container mx-auto px-6">
                    <div className="grid md:grid-cols-4 gap-10">
                        ...
                        {/* Contacts */}
                        <div>
                            <h4 className="text-sm uppercase tracking-wider mb-4">{t('contacts')}</h4>
                            <div className="space-y-3">
                                ...
                                {/* Maps Icon - shows only when link exists */}
                        ...
                        {settings?.schedule_enabled && settings?.schedule && (
                            <div>
                                <h4 className="text-sm uppercase tracking-wider mb-4">{t('schedule')}</h4>
                        ...
                        {(settings?.delivery_price || settings?.pickup_info || settings?.delivery_info || settings?.payment_info) && (
                            <div>
                                <h4 className="text-sm uppercase tracking-wider mb-4">{t('information')}</h4>
                    ...
                    <div className="border-t border-zinc-800 mt-12 pt-8 text-center">
                        <p className="text-zinc-500 text-sm">
                            © 2026 {settings?.shop_name || 'Diana Flowers'}. {t('allRightsReserved')}
                        </p>
                    </div>
                </div>
            </footer>
