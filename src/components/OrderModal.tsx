...
import { X, Truck, Store, Clock, CheckCircle, Loader2, CreditCard } from 'lucide-react';
...
export default function OrderModal({ product, isOpen, onClose, deliveryEnabled = true, settings }: OrderModalProps) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            // Redirect to payment if payment_url exists
                    ...
                    >
                        ...
                        <div className="sticky top-0 bg-[#FFFFF0] p-5 border-b border-gray-200 flex justify-between items-center z-10">
                            <h2 className="font-serif text-xl sm:text-2xl font-semibold text-[#1a1a1a]">
                                {step === 4 ? 'Заказ оформлен!' : step === 1 ? 'Выбранный букет' : step === 2 ? 'Способ получения' : 'Контактные данные'}
                            </h2>
                        ...
                        <div className="p-5 sm:p-6">
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div>
                                        <h4 className="font-semibold mb-3 text-[#1a1a1a]">
                                            {deliveryType === 'delivery' ? 'Когда доставить?' : 'Когда заберёте?'}
                                        </h4>
                                        ...
                                        {timeType === 'specific' && (
                                            <div className="mt-4 grid grid-cols-2 gap-3">
                                                <div>
                                                    <label className="block text-sm font-medium mb-1 text-gray-700">День</label>
                                                    <select
                                                        value={selectedDay}
                                                ...
                                                <div>
                                                    <label className="block text-sm font-medium mb-1 text-gray-700">Время</label>
                                                    <select
                                                        value={selectedTime}
                            ...
                            {step === 4 && (
                                <div className="text-center py-8">
                                    ...
                                    >
                                        {product.payment_url ? (
                                            <Clock size={80} className="text-[#D4AF37] mx-auto mb-4" />
                                        ) : (
                                            <CheckCircle size={80} className="text-green-500 mx-auto mb-4" />
                                        )}
                                    </motion.div>
                                    <h3 className="font-serif text-2xl font-semibold mb-2">
                                        {product.payment_url ? 'Оплата в процессе' : 'Спасибо!'}
                                    </h3>
                                    <p className="text-gray-600 mb-6">
                                        {product.payment_url
                                            ? 'Ваш заказ принят, ожидайте! Если окно оплаты не открылось, нажмите кнопку ниже.'
                                    ...
                                    {product.payment_url && (
                                        <a
                                            ...
                                            className="inline-flex items-center gap-2 bg-[#D4AF37] text-black py-3 px-8 rounded-xl font-semibold hover:bg-[#E8C878] transition-colors mb-3"
                                        >
                                            <CreditCard size={20} />
                                            Перейти к оплате
                                        </a>
                                    ...
                                    <button
                                        onClick={resetAndClose}
                                        className="bg-[#1a1a1a] text-white py-3 px-8 rounded-xl font-semibold hover:bg-[#333] transition-colors mt-2"
                                    >
                                        Закрыть
                                    </button>
                                </div>
