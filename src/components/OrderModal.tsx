...
export default function OrderModal({ product, isOpen, onClose }: OrderModalProps) {
    ...
    const handlePhoneChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        // Prevent typing more than 11 digits (7 + 10 digits)
        if (value.length > 11) value = value.substring(0, 11);

        setFormData(prev => ({ ...prev, phone: value }));
        // Clear error if valid length (11 digits for RU)
        if (value.length === 11) {
    ...
    const handleSubmit = async () => {
        ...
        // Custom validation for exact 11 digits
        if (!/^\d{11}$/.test(formData.phone)) {
            setErrors({ phone: 'Введите номер полностью (11 цифр, пример: 89933337710)' });
            return;
        }
        ...
        try {
            const orderData: OrderData = {
                ...
                productId: String(product.id), // ensure productId is string
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div>
                                        ...
                                        <input
                                            ...
                                            className={`w-full p-3 border rounded-xl focus:border-soft-gold focus:outline-none transition-colors ${errors.phone ? 'border-red-500 bg-red-50' : 'border-gray-200'
                                                }`}
                                        />
                                        {errors.phone && (
